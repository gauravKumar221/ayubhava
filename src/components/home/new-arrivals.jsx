...
export function NewArrivals() {
  ...
  const handleAddToCart = (product) => {
    ...
    toast({
      title: "Added to Cart!",
      description: `${product.title} added to your ritual.`,
    });
  };

  // Showcase recent additions
  const newProducts = products.filter(p => p.category !== "Best Sellers").slice(0, 4);
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 bg-accent/10 text-accent px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">
          ...
          <Button asChild variant="link" className="font-black uppercase text-xs tracking-widest p-0 h-auto">
            <Link href="/products">Explore Latest</Link>
          </Button>
        </div>
                ...
                <div key={product.id} className="flex flex-col group h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="relative mb-4 md:mb-6">
                    <Link href={`/products/${product.id}`} className="block">
                      <div className="relative aspect-square bg-[#f9f9f9] rounded-2xl overflow-hidden shadow-sm border border-black/5 transition-all hover:shadow-xl">
                        <LazyImage 
                          src={media?.imageUrl} 
                          alt={product.title} 
                          fill 
                        ...
                        />
                        <div className="absolute top-3 left-3 bg-black text-white px-2 py-1 text-[8px] font-black uppercase tracking-widest rounded-sm z-10">
                          New
                        </div>
                      </div>
                    </Link>
                  ...
                  <div className="flex-1 flex flex-col space-y-3 px-1">
                    ...
                    <div className="mt-auto pt-2 space-y-4">
                      <span className="text-lg md:text-xl font-black block">₹{product.price}</span>
